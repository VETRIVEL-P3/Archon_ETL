...
//	@Autowired
//	private AuditHelperFC auditHelper;
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!HttpMethod.POST.name().equals(request.getMethod())) {
            ...
            throw new ServerException("Authentication method not supported");
        }

        LoginRequest loginRequest = objectMapper.readValue(request.getReader(), LoginRequest.class);

        if (StringUtils.isEmpty(loginRequest.getUserId()) || StringUtils.isEmpty(loginRequest.getPassword())) {
            throw new AuthenticationServiceException(FailureMessages.MISSING_CREDENTIALS);
        }

        /*
         * if (!Validators.email(loginRequest.getUserId())) { throw new
         * AuthenticationServiceException(FailureMessages.INVALID_EMAIL_FORMAT); }
         */


        UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(loginRequest.getUserId(),
